...
def daily_reminder_mail(request):
    try:
        ...
        # Query the different dues in the next 3 days
        ...
        def format_dues(dues, date_field, due_name):
            return [{'vehicle_id': due['vehicle__id'], 'vehicle_number': due['vehicle__vehicle_number'], 'due_date': due[date_field], 'due_name': due_name} for due in dues]

        three_days_later_dues = (
            format_dues(emi_dues, 'next_due_date', 'EMI') +
            format_dues(policy_dues, 'due_date', 'Policy') +
            format_dues(tax_dues, 'tax_due_date', 'Tax') +
            format_dues(fitness_dues, 'fitness_due_date', 'Fitness') +
            format_dues(permit_dues, 'permit_due_date', 'Permit') +
            format_dues(puc_dues, 'puc_due_date', 'PUC')
        )

        # Query past due payments (expired dues)
        twelve_months_ago = today - timedelta(days=365)
        one_day_ago = today - timedelta(days=1)

        emi_dues = EMI.objects.filter(next_due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active", status='pending').values('vehicle__id', 'vehicle__vehicle_number', 'next_due_date')
        policy_dues = Policy.objects.filter(due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active").values('vehicle__id', 'vehicle__vehicle_number', 'due_date')
        tax_dues = OtherDues.objects.filter(tax_due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active").values('vehicle__id', 'vehicle__vehicle_number', 'tax_due_date')
        fitness_dues = OtherDues.objects.filter(fitness_due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active").values('vehicle__id', 'vehicle__vehicle_number', 'fitness_due_date')
        permit_dues = OtherDues.objects.filter(permit_due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active").values('vehicle__id', 'vehicle__vehicle_number', 'permit_due_date')
        puc_dues = OtherDues.objects.filter(puc_due_date__range=[twelve_months_ago, one_day_ago], vehicle__status="active").values('vehicle__id', 'vehicle__vehicle_number', 'puc_due_date')
...
def vehicle_dashboard(request,id):
    try:
        ...
        other_dues_data=OtherDues.objects.select_related('vehicle').filter(vehicle=vehicle) 
        ...
        emi=EMI.objects.select_related('finance_bank','vehicle').filter(vehicle=vehicle).first() 

        policy=Policy.objects.select_related('insurance_company','vehicle').filter(vehicle=vehicle).first()
...
def delete_emi(request, id):
    try:
        ...
        messages.error(request, f"Error deleting EMI: {str(e)}")
...
def emi_item_list(request, id):
    ...
    # Fetch the EMI object
        ...
        emi_items = EMI_Item.objects.select_related('emi').filter(emi=emi)
